<template>
    <li>
          <div class=" divide-y divide-gray-200 rounded-lg bg-white shadow overflow-hidden">
            <div class="p-6 space-y-6">
              <div class="flex items-center justify-between">
                <h3 class="text-3xl font-semibold">next-nested-routing</h3>
                <a href="#" 
                class="border rounded-md border-gray-200 px-5 font-medium py-1.5 text-gray-600 text-sm hover:border-black trasition ease-in-out duration-150">
                Visit
                </a>
              </div>
              <div class="space-y-3">
                <div aria-label="Production Deployment" class="flex items-center space-x-3">
                  <a href="#" class="mt-px inline-flex items-center space-x-2 text-sm leading-5 font-medium">
                    <span aria-hidden="true" class="h-2.5 w-2.5 rounded-full bg-green-300 inline-block"></span>
                    <span>shamelesscomfort.com</span>
                  </a>
                  <span class="inline-flex rounded-full bg-gray-50 border border-gray-200 px-1.5 py-px text-xs leading-5 font-medium">Production</span>
                <span class="text-sm leading-5 text-gray-500">
                  <time datetime="2019-10-29">
                    204d
                  </time>
                </span>
                </div>
                <div aria-label="Latest Deployment" class="flex items-center space-x-3">
                  <a href="#" class="mt-px inline-flex items-center space-x-2 text-sm leading-5 font-medium">
                    <span aria-hidden="true" class="h-2.5 w-2.5 rounded-full bg-green-300 inline-block"></span>
                    <span>next-nested-routing-5ipzp1m88.now.sh</span>
                  </a>
                  <span 
                  class="inline-flex rounded-full bg-gray-50 border border-gray-200 px-1.5 py-px text-xs leading-5 font-medium">
                  Latest
                  </span>
                <span class="text-sm leading-5 text-gray-500">
                  <time datetime="2020-04-04">
                    46d
                  </time>
                </span>
                </div>
            </div>
            </div>
            <div class="px-6 py-3">
              <a 
              href="#" 
              class="text-sm leading-s font-medium inline-flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" style="-ms-transform: rotate(360deg); -webkit-transform: rotate(360deg); transform: rotate(360deg);" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
                <g fill="black">
                <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
                </g>
                <rect x="0" y="0" width="32" height="32" fill="rgba(0, 0, 0, 0)" /></svg>
                <span>kojusola/next-nested-routing</span>
              </a>
            </div>
          </div>
        </li>
</template>

<script>
export default {
    
}
</script>
<style  scoped>

</style>